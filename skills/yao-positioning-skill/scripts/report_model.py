...
def compute_metrics(data: dict[str, Any]) -> dict[str, Any]:
    ...
    valid_differences = sum(
        (item.get("relevance") or 0) >= 4
        and (item.get("rarity") or 0) >= 3
        and (item.get("evidence_strength") or 0) >= 3
        and item.get("status") == "eligible"
        for item in data.get("advantages", [])
    )
    key_risks = sum(
        (option.get("risk_level") or 0) >= 4 for option in data.get("positioning_options", [])
