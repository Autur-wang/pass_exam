...
> ⚠️ **当前状态（2026-07 校准）**：本目录下所列的 15 个子项目**内容尚未落盘**——各子目录目前为空占位，`.git`、源码、构建文件均不存在。下文是**计划中的项目目录清单（待克隆 / 待创建）**，描述的是这些项目建成后的形态，而非磁盘现状。在对应目录出现实际内容前，请把本文当作「待办克隆清单」，不要据此假设任何文件已存在。

> 本文件供 AI coding agent 阅读。当前目录 `/Users/bytedance/pass_exam/个人情况与简历` 是一个**多项目工作区**，每个子目录都是独立的 Git 仓库，分别对应简历、面试、求职、系统架构等不同的开源项目或 AI Skill 包。
>
